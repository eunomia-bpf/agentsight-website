...
- PR `#119` had a patrol finding at head `159df59b11917cb9ce2546f604841a9cbad71000`: the TLS refresh used a dedicated renderer while the public-content registry still carried stale v1.0.3 page data for the same canonical slug, and this file had not been refreshed for the 9 September cycle. Final head `d3a0bc924a86786ca1933a41245c903549c1baf5` consolidated the TLS article back into one `ContentPage` registry record, removed the temporary route/hub substitution glue, and refreshed operating metadata. Exact-head Website CI run `34400101439` / job `102629572891`, GitGuardian, and Cloudflare preview `https://65fbb655.agentsight.pages.dev` succeeded. PR `#119` was squash-merged as `a2acfac71e75e7ee41539c5acd583c1605d1a0bd`; exact `Publish static site` run `34400475435` succeeded from that commit; production `site/.source-sha` matches; live verification at `2026-09-09T20:22:57Z` found one TLS H1, one canonical link, no standalone TLS `v1.0.3` marker, the v1.0.31 Electron/Cursor, `k8s://`, `browsertrace`, and `stdiocap` boundaries, one Blog hub TLS card, unchanged homepage v1.0.31 footer, and 2026-09-09 sitemap freshness for `/blog/` and the TLS route.
- A 10 September independent crawler recheck directly retrieved `/blog/why-ai-agent-tls-traffic-is-hard-to-trace/` and showed `refreshed September 2026 · AgentSight v1.0.31` plus the expected current TLS diagnostic content. The temporary direct-retrieval qualification after the 9 September publication is therefore cleared; no cache-forcing repair is needed.
